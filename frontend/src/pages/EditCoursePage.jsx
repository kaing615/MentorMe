...
const EditCoursePage = () => {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      const { response, error } = await courseApi.getCourseDetails({ courseId });
      if (response && response.data) {
        ...
        setValue('lectures', course.lectures);
        setValue('courseOverview', course.description || '');
        setValue('keyLearningObjectives', course.shortDescription || '');
        setValue('driveLink', course.link || '');
        setValue('duration', course.duration);
        setImagePreview(course.thumbnail ? (course.thumbnail.startsWith('http') ? course.thumbnail : `http://localhost:4000/${course.thumbnail}`) : null);
      } else {
  ...
  const onSubmit = async (data) => {
    try {
      // Validate image is required (optional for edit, only if user changes)
      // If no imageFile, keep old image
      ...
      const { response, error } = await courseApi.updateCourse({ courseId, courseData: formData });
      if (response) {
