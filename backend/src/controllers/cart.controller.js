...
const getCart = async (req, res) => {
  try {
    ...
    if (!cart) {
      cart = new Cart({ user: userId, courses: [], totalPrice: 0 });
      await cart.save();
    }

    // Tính lại total price
    const totalPrice = cart.courses.reduce((sum, item) => {
      return sum + (item.course?.price || 0);
    }, 0);

    // Cập nhật total price nếu khác
...
const addToCart = async (req, res) => {
  try {
    ...
    // Validate course ID
    if (!courseId) {
      return responseHandler.badrequest(res, "Course ID là bắt buộc.");
    }
...
const removeFromCart = async (req, res) => {
  try {
    ...
    if (!cart) {
      return responseHandler.notfound(res, "Không tìm thấy giỏ hàng.");
    }
...
const checkInCart = async (req, res) => {
  try {
    ...
    let inCart = false;
    if (cart) {
      inCart = cart.courses.some((item) => item.course.toString() === courseId);
    }
