...
// Tạo order từ cart hoặc từ data trực tiếp
export const createOrder = async (req, res) => {
  try {
    const orderData = req.body;
    const userId = req.user._id;

    // New format: order data passed directly
    if (orderData && orderData.courses && Array.isArray(orderData.courses)) {
      // Validate courses exist
      const validatedCourses = [];
      let subtotalAmount = 0;

      for (const courseData of orderData.courses) {
        const course = await Course.findById(
          courseData.courseId || courseData._id
        );
        if (!course) {
          return responseHandler.badRequest(res, `Khóa học không tồn tại!`);
        }
        ...
        validatedCourses.push({
          ...
          title: course.title,
          price: course.price,
          quantity: 1, // Course always quantity 1
      ...
      const newOrder = new Order({
        ...
        mentee: userId,
        userId: userId,
        items: validatedCourses,
        courses: validatedCourses.map((c) => c.courseId),
        type: "course",
        subtotalAmount,
        discountAmount: orderData.discountAmount || 0,
        amount: orderData.totalAmount || subtotalAmount,
        totalAmount: orderData.totalAmount || subtotalAmount,
    ...
    // Legacy format: from cart with billing info
...
export const getUserOrders = async (req, res) => {
  try {
    ...
    // Đếm tổng số orders
    let countQuery = {
      $or: [{ userId: userId }, { mentee: userId }],
    };
...
// Cập nhật trạng thái order (admin hoặc user)
export const updateOrderStatus = async (req, res) => {
  try {
    ...
    let order;
    if (orderId) {
      // Find by ID (user route)
      order = await Order.findById(orderId);
    } else {
      // Find by order number (admin route)
      ...
      orders: orders.map((order) => ({
        ...
        customer: {
          name:
            ...
            `${order.mentee?.firstName || ""} ${
              order.mentee?.lastName || ""
            }`.trim(),
